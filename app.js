...
const BUILD_VERSION = '2026-06-03 23:47';
...
if (act === 'gojuon') {
  ...
  let imgViewerIsOpen = false;

  function openImgViewer(url) {
    if (!url || !el.imgOverlay || !el.imgViewerImg) return;

    // ✅ 2重に開いて ScrollLock が積み上がるのを防ぐ
    el.imgViewerImg.src = url;
    if (imgViewerIsOpen && !el.imgOverlay.classList.contains('isHidden')) return;

    imgViewerIsOpen = true;
    ScrollLock.lock(); // ✅
  ...
  function closeImgViewer() {
    ...
    const wasOpen = imgViewerIsOpen && !el.imgOverlay.classList.contains('isHidden');

    el.imgOverlay.classList.add('isHidden');
    if (el.imgViewerImg) {
      el.imgViewerImg.removeAttribute('src');
      el.imgViewerImg.src = '';
    }
    ...
    function renderRhynioManagerList() {
      ...
      rhynioSlots
        ...
        .forEach((rawSlot) => {
          ...
          soldChk?.addEventListener('click', async (e) => {
            e.preventDefault();
            const target = rhynioSlots.find(x => Number(x.id) === Number(slot.id));
            if (!target) return;
          ...
          del.addEventListener('click', async () => {
            ...
            const hasMeta = !!String(target?.level || '').trim() || !!target?.soldOut || String(target?.statType || 'M') !== 'M';
            if (!hasImg && !hasMeta) {
